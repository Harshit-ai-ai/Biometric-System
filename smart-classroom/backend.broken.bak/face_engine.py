...
<<<<<<< HEAD
            ...
            # real person's hands/torso, playing back the enrolled
            # person's photo or video. Reuses the face box already found
            # above instead of re-running face detection.
            screen_check = screen_spoof_engine.analyze_face_box(image, (top, right, bottom, left))
            if screen_check and screen_check["spoof_score"] >= screen_spoof_engine.SPOOF_THRESHOLD:
                # Looks like a displayed image, not a real face — treat
                # like any other spoof and skip this detection.
                continue

            # Level 4: dedicated presentation-attack model, if one has been
            # exported (see liveness/pad_model.py + scripts/export_pad_model.py).
            # Fails open by design: when no model is loaded this is a no-op
            # and detection proceeds exactly as it did before Level 4 existed.
            # When a model IS loaded, it only skips the detection on a
            # confident non-live prediction — an "uncertain" verdict doesn't
            # block anything, since Levels 1/3 already did the real gating.
            pad_check = pad_detector.predict_face_box(image, (top, right, bottom, left))
